...
st.plotly_chart(fig)
#plt.plot(lstm_hist['loss'], label = 'Потери на тренировочной выборке')
#plt.plot(lstm_hist['val_loss'], label = 'Потери на тестовой выборке')
#plt.legend()
#plt.xlabel('Эпоха')
#plt.ylabel('Ошибка')
#plt.title('Потери при обучении модели (MAE)')
#st.pyplot(plt.gcf())
